...
__all__ = ('CudaProjectorGain', )

class CudaProjectorGain(odl.Operator):
    """ A projector that creates several projections as defined by geometries
    """
    def __init__(self, volume, nVoxels, geometry, gain,
        ...
        # Set locals
        self.gain = projectionDisc.element(gain.copy())
        self._corr_gain = self.gain.copy()
    ...
    def _call(self, data, out):
        ...
        for out_i, geo in zip(out, self.geometries):
            for i in range(len(self.spectrum)):
                self._corr_gain.lincomb(self.spectrum[i], self.gain)
                ...
                self.forward.project(geo.sourcePosition,
                                     ...
                                     self._corr_gain.ntuple.data_ptr,
                                     self.energy_min[i],
                                     self.energy_max[i])
...
if __name__ == '__main__':
    ...
    projector = CudaProjectorGain(volume, nVoxels, geom, gain,
