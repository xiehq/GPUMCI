...
class CudaSimpleMCProjector(odl.Operator):
    ...
    def __init__(self, volumeOrigin, voxelSize, nVoxels, nPixels, geometries, domain, range, nphotons):
        ...
        with Timer('__init__'):
            self.forward = gpumci.SpectMC(nVoxels, volumeOrigin, voxelSize, nPixels, nphotons, materials, rayleighTables, comptonTables)
...
plt.imshow(dose_result.asarray()[:,:,nVoxels[2]//2], cmap='hot')
plt.colorbar()
