...
def phantom3d(phantom='modified_shepp_logan', n=[64,64,64]):
    ...
    ==========
    phantom: str
        One of {'modified_shepp_logan', 'shepp_logan', 'yu_ye_wang'},
    ...
    """
    if phantom == 'modified_shepp_logan':
    ...
    n = np.asarray(n)
    p = np.zeros(np.prod(n))

    x, y, z = np.meshgrid(np.linspace(-1,1,n[0]), np.linspace(-1,1,n[1]), np.linspace(-1,1,n[2]))
    ...
    for k in range(ellipse.shape[0]):
        ...
        c3 = np.cos(psi)
        s3 = np.sin(psi)
        zero = np.array([x0,y0,z0])

        # Euler rotation matrix
        alpha = np.array([[c3 * c1 - c2 * s1 * s3,
                           c3 * s1 + c2 * c1 * s3,
                           s3 * s2],
                          [-s3 * c1 - c2 * s1 * c3,
                           -s3 * s1 + c2 * c1 * c3,
        ...
        coordp = np.dot(alpha, (coord.T-zero.T).T).T
        idx = np.sum(coordp ** 2.0 / np.array([asq,bsq,csq]).T, axis=1) <= 1
        p[idx] += A
...
def shepp_logan():
    ...
    arr[:, 0] = np.array([2, -0.98, -0.02, -0.02, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01])
    return arr
