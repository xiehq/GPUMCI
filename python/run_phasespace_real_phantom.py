...
def getMaterialInfo():
    ...
    return gpumci.MaterialData(energyStep,compton*0,rayleigh,photo*0)
...
def getPhaseSpace():
    phase_space = gpumci.particle_array()
    position = np.array([-790.0,0.0,0.0])
    a = 0.008
    f = 0
    dr = 1.0/10000
    for t in np.linspace(0.0,1.0,10000):
        f += dr / (a*np.sqrt(1+f**2))
        theta = f
        r = a*f
        direction = np.array([1.0,np.cos(theta)*r,np.sin(theta)*r+0.12])
        direction = direction / np.linalg.norm(direction)
        phase_space.append(gpumci.particle(position, direction, 0.1, 1.0))
...
class CudaSimpleMCProjector(odl.Operator):
    ...
    def __init__(self, volumeOrigin, voxelSize, nVoxels, nPixels, geometries, domain, range, nphotons):
        ...
        with Timer("getPhaseSpace"):
            self.ps = getPhaseSpace()
        materials = getMaterialInfo()
        rayleighTables = getRayleighTables()
        comptonTables = getComptonTables()
        with Timer('__init__'):
            self.forward = gpumci.PhaseSpaceMC(nVoxels, volumeOrigin, voxelSize, nPixels, nphotons, materials, rayleighTables, comptonTables)
        ...
        for out_i, geo in zip(out, self.geometries):
            for i in range(10):
                with Timer("projecting"):
                    self.forward.project(geo.sourcePosition, geo.detectorOrigin, geo.pixelDirectionU, geo.pixelDirectionV, self.ps, out_i[0].data_ptr, out_i[1].data_ptr)
