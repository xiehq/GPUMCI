...
def getMaterialInfo():
    ...
    return gpumci.MaterialData(energyStep,compton,rayleigh*0,photo*0)
        ...
        for out_i, geo in zip(out, self.geometries):
            for i in range(10):
                with odl.util.Timer("projecting"):
                    self.forward.project(geo.sourcePosition, geo.detectorOrigin, geo.pixelDirectionU, geo.pixelDirectionV, out_i[0].ntuple.data_ptr, out_i[1].ntuple.data_ptr)
