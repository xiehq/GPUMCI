...
class CudaSimpleMCProjector(odl.Operator):
    ...
    def __init__(self, volumeOrigin, voxelSize, nVoxels, nPixels, geometries, domain, range, nphotons):
        ...
        with Timer('__init__'):
            self.forward = gpumci.PrecomputedMC(nVoxels, volumeOrigin, voxelSize, nPixels, nphotons, materials, rayleighTables, comptonTables)
...
detectorSize = np.array([500.0,500.0])
...
nVoxels, nPixels = np.array([512, 465, 65]), np.array([500, 500])
nProjection = 1
nphotons = 300
...
geometries = []
for theta in np.linspace(np.pi/2.0, 2*pi+np.pi/2.0, nProjection, endpoint=False):
...
#Create a phantom
phantomVec = odl.util.shepp_logan(reconDisc)
materialsVec = reconMatDisc.element(np.ones(nVoxels))
