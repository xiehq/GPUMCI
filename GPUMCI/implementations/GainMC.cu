...
namespace cuda {
namespace {
//Find the number of threads needed
...
//stored here as to not expose cuda to the outside
struct GainMCCuData {
    GainMCCuData(const int3 volumeSize,
    ...
    //Nocopy
    GainMCCuData(const GainMCCuData&) = delete;
    GainMCCuData& operator=(const GainMCCuData&) = delete;
...
GainMC::GainMC(const Eigen::Vector3i& volumeSize,
    ...
    _cudaData = std::make_shared<cuda::GainMCCuData>(make_int3(volumeSize),
...
void GainMC::setData(const float* densityDevice,
...
void GainMC::project(const Eigen::Vector3d& sourcePosition,
