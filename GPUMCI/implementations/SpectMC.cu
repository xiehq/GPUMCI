...
#include <GPUMCI/photongenerator/PhotonGeneratorSPECT.cuh>
...
namespace cuda {
namespace {
unsigned nThreads(int3 volumeSize) {
...
struct SpectMCCuData {
    SpectMCCuData(const int3 volumeSize,
    ...
    //Nocopy
    SpectMCCuData(const SpectMCCuData&) = delete;
    SpectMCCuData& operator=(const SpectMCCuData&) = delete;
...
SpectMC::SpectMC(const Eigen::Vector3i& volumeSize,
    ...
    _cudaData = std::make_shared<cuda::SpectMCCuData>(make_int3(volumeSize),
...
void SpectMC::setData(const float* densityDevice,
...
void SpectMC::project(const Eigen::Vector3d& sourcePosition,
    ...
    unsigned numberOfThreads = cuda::nThreads(_param.volumeSize);
    ...
    //Use a uniform spect generator
    cuda::PhotonGeneratorSPECT photonGenerator{narrow_cast<unsigned>(_nRuns)};
